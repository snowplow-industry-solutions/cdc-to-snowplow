...
@Testcontainers
class TransformTypeCheckFailureTest {
    ...
    @Test
    fun `start() throws StartupException when a transform targets a numeric column`() {
        // The init script creates public.orders.total as NUMERIC(10, 2). Postgres reports its
        // data_type as "numeric", which is outside TransformTypeCheck's string allowlist
        // {text, character varying, character}. Configuring `trim` on it must fail startup
        // with a column-named StartupException.
        ...
        e.message!! shouldContain "public.orders.total"
        e.message!! shouldContain "\"numeric\""
        e.message!! shouldContain "\"trim\""
        e.message!! shouldContain "text / character varying / character"
    }
}
